...
**doc-rag** 是一个本地文档检索与问答系统：Java + Lucene 后端负责 docx / xlsx / pdf 的解析与**双模式索引入库**，检索采用 BM25（可选融合 vector-service 语义召回）；检索命中的片段可送 **OpenAI 兼容 LLM** 生成带引用文档列表的问答；Python Flask Web 前端提供搜索 + 内嵌问答 + 索引明细查看页面，命中片段高亮展示。
...
**两模式结构完全对称**：各自一套 Lucene 倒排（`data/index-plain/`、`data/index-deep/`）+ 各自一个向量 collection，检索与问答均按模式选择（可选 1-2 个，双模式时检索结果分模式返回、前端两栏对比）。
...
- `IngestService`：入库编排（从 Controller 抽出）——按选中模式解析（fail-fast，全部解析成功才写库）→ per-mode 切块（plain=`Chunker.chunk`；deep=`Chunker.chunkKeepingTables(content, DEEP_EMBED_MAX_CHARS=256)`，注意与问答用的 512 是不同用途常量）→ 写库顺序 plain 倒排 → deep 倒排 → plain 向量 → deep 向量，`List<Runnable> rollbacks` 逆序回滚（回滚失败 log.warn 不吞原始异常）。
...
- `ModeSearcher`：plain 与 deep 共用（原 DocumentSearcher/TableSearcher 合并；deep 也走混合检索）——BM25（`MultiFieldQueryParser` 查 `filename`+`content`）与 vector-service 语义召回（`query(mode, q, topK)` 查对应 collection）两路，docId 级 RRF 融合，向量不可用**该模式**降级纯 BM25（per-mode `degraded=true`）；`Highlighter`+`SimpleFragmenter` 截取最佳片段，命中词 `<em>` 包裹；`getById(docId)` 取该模式入库文本；`topDocsByDocIds(q, docIds, topN)` 供问答在选中文件范围内 BM25 召回；`listAll()` 供明细页。
...
- `AskService` 编排：选中 docIds + modes（默认 `["plain"]`）→ **合并上下文单次 LLM 调用**——各模式独立：BM25 召回选中文档（`topDocsByDocIds`）→ 查询时切块（plain 用 `Chunker.chunk`；deep 用 `Chunker.chunkKeepingTables`，markdown 表格块含标题行整体保留）→ 按与问题的分词重叠度排序。**双模式配额**：每模式 chunk 配额 = `max(2, maxContextChunks/2)`、字符预算减半；单模式全额。拼编号 prompt（资料头行带模式标签：`[n] filename · 纯文本解析` 或 `[n] filename · 深度解析 · 表格 N`；system：仅依据资料作答、引用标 [n]、资料不足须明说）→ 调 LLM → 返回 `answer + citations`（每个上下文块一条：ref/docId/filename/type/**mode**/title/excerpt；ref 全局连续，plain 先 deep 后，title 仅 deep 填表格标题）。某模式无命中另一模式有 → 照常作答；全部为空 → 返回「无法作答」类提示，不调 LLM。
...
- 请求：`{"question": "试用期最长多久？", "docIds": ["uuid1", "uuid2"], "modes": ["plain", "deep"]}`
- `docIds` 必填（界面上勾选的文档），`modes` 默认 `["plain"]`；双模式合并上下文一次 LLM 调用
- 响应：
```json
{
  ...
  "modes": ["plain", "deep"],
  "citations": [
    {"ref": 1, "docId": "uuid1", "filename": "xx.docx", "type": "docx", "mode": "deep", "title": "表格 2", "excerpt": "| 试用期 | 6 个月 |…"}
  ]
}
```
- LLM 未配置返回 400；选中范围内所有模式都检索不到内容时直接返回「无法作答」类提示，不强行调用 LLM
