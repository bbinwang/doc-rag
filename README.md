...
- 检索选双模式时，结果按模式分两栏对比展示；问答为独立页面（`/ask`），双模式时两栏各自作答——每模式独立走 chunk 级混合检索（BM25 + 向量 RRF）并各自调用一次 LLM。
...
| 页面 | 路径 | 说明 |
|---|---|---|
| 搜索页 | `/` | 上传文件（可选模式）、关键词检索（可选 1-2 模式，双模式分栏对比）、结果高亮 |
| 问答页 | `/ask` | 独立问答（全库检索、不选文档）：问题 + 模式 + 三参数（倒排/向量/送模型 chunk 数，默认 5/5/8）→ 双模式两栏答案 + 召回 chunks/文档明细 |
...
### 问答流程（独立页面 /ask，每模式独立执行）

```
用户输入问题 + 模式 + 三参数（倒排/向量/送模型 chunk 数，默认 5/5/8）
  → 每模式: BM25 全库 doc 召回（pool 5-20 篇）→ 入库同款切块 → 分词重叠度排序取前 N chunk
  → 每模式: VectorClient 语义召回 topK=N（查该模式 collection，如可用）
  → 两路按 (docId, chunk 文本) 去重 → chunk 级 RRF 融合 (k=60)
  → 按 contextChunks + 字符预算截断 → 拼编号 prompt → 该模式独立调 LLM
  → 两栏返回：每模式 答案 + chunks（ref 对应 [n]）+ 去重文档
```
